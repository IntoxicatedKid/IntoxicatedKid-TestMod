        ...
        public override IEnumerable<BattleAction> GetPassiveActions()
        {
            ...
            for (int i = 0; i < Battle.FriendPassiveTimes; i = num + 1)
            {
                ...
                if (Rng1 > 2)
                {
                    ...
                    {
                        case 0:
                            List<Card> list1 = Battle.HandZone.Where((Card card) => !card.IsPurified && card.Cost.HasTrivial).ToList();
                            if (list1.Count > 0)
                            {
                            ...
                            {
                                List<Card> list2 = Battle.HandZone.Where((Card card) => !card.IsPurified).ToList();
                                if (list2.Count > 0)
                                {
            ...
            if (Rng4 > 2)
            {
                switch (Rng5)
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        yield return new UpgradeCardsAction(Battle.HandZone.Where((Card card) => card.CanUpgradeAndPositive));
                        break;
                    case 3:
